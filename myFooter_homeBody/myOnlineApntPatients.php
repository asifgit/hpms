<?php
	...
	$appntOnlinePtResults	= mysqli_query($con, "SELECT * FROM hmsaaaappointments WHERE APDate='$dateBuilderString' AND APDealt=false ORDER BY APTime");
		...
		echo'
			<tr>
				<td style="text-align:center; font-style:italic;" colspan="8">
					No Online Appointments to be scheduled, today.
				</td>
			</tr>
		';
	}
		...
		while ($row_user = mysqli_fetch_assoc($appntOnlinePtResults))
		{
			...
			while($row_userPtUser 	=	mysqli_fetch_assoc($usrNames))
			{
				...
				echo'
					<tr id = "' . $row_user['APID'] . '">
						<td style="text-align:center;">
							<span class="badge" style="margin:0px; background-color:#B6B6B2;">' . $countlist . '</span>
						</td>
						<td style="text-align:center;">
						...
						<td style="text-align:left;">
							<p id="' . $row_user['APID'] . '" data-toggle="modal" style="text-decoration:none; margin:0px; color:BLACK;">
								&emsp;&emsp;' . $row_userPtUser['UFNaam'] . ' ' . $row_userPtUser['ULNaam'] . '
							</p>
						</td>
				...
				echo'
						...
						<td style="border-left:none;" id = "haha' . $row_user['APID'] . '">
							&emsp;
							<a style="color:rgb(79, 205, 27);" href="#" onclick="assignTkn(' . $row_user['APID'] . ');">Assign</a> / 
							<a style="color:rgb(242, 26, 43);" href="#" onclick="cancelTkn(' . $row_user['APID'] . ');">Cancel</a>
							Token
						</td>
						<td>
							&emsp;&emsp;' . $row_userDcUser['USpeciality'] . '
						</td>
					</tr>
				';
				echo'
