...
(function(){
	var 
		...
		b.prototype={
			...
			j:function(b,d){
				var 
				c=document.getElementById(b.TargetId);
				if(!c)
					return;
				this.l(c);
			...
			m:function(){
				...
				for(var c=this,a=0;a<this.b.length;a++){
					var 
					b=document.getElementById(this.b[a].TargetId);
					if(b){
						this.c.push(b);
						d(
							this.b[a],
			...
			n:function(){
				var 
				a=this.d?parseInt(this.h()):this.i();
				if(a>=this.b.length)
					a=0;this.j(this.b[a],0)
			}
		};
		var 
		k=[],
